...
Model is divided into four parts: Entity, Repository and Facade. There is `EntityManager` to access the database.
...
class CartFacade
{
    ...
    public function addProductToCart($productId, $quantity)
    {
        ...
        //TODO fix this calling as CartService is removed
...
The controller handles the request (eg. saved data from form) and passes data to the facade. The facade receives data from the controller, requests appropriate entities from the repository and (if needed) passes all those data to the entity for processing. Entity processes data and returns output to the facade, that persist it by entity manager.
